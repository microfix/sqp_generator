...
export const generatePDF = async (
  ...
  try {
    ...
    // Calculate estimated number of TOC pages
    const initialPageHeight = 841.89; // A4 height
    const startY = initialPageHeight - 80; // Starting Y position for first entry
    const entryHeightLevel0 = 25; // Height for level 0 entry
    const entryHeightLevel1 = 20; // Height for level 1+ entry
    const bottomMargin = 50; // Bottom margin

    let estimatedTocEntryCount = 0;
    const tempTocEntries: { title: string, page: number, level: number, showPage: boolean }[] = [];

    const estimateNestedSection = (section: any, level: number = 0) => {
        if (section.showInToc) {
            tempTocEntries.push({ title: section.title, page: 0, level: level, showPage: section.showInToc });
    ...
    for (const entry of tempTocEntries) {
        const entryHeight = entry.level === 0 ? entryHeightLevel0 : entryHeightLevel1;
        if (currentEstimatedY - entryHeight < bottomMargin) {
            estimatedTocPagesNeeded++;
            currentEstimatedY = initialPageHeight - 50; // Reset Y for new page
        }
    ...
    for (let i = 0; i < estimatedTocPagesNeeded; i++) {
        pdfDoc.insertPage(tocStartPage + i, [595.28, 841.89]); // Insert A4 pages
        currentPage++;
    }
    ...
    let currentTocPageIndex = 0; // Track which TOC page we are on
    let tocPage = pdfDoc.getPages()[tocStartPage + currentTocPageIndex]; // Get the first TOC page
    ...
    for (const entry of tocEntries) {
        // Adjust page number for the inserted TOC pages
        ...
        if (yPosition < 50) {
            ...
            if (currentTocPageIndex < estimatedTocPagesNeeded) {
                ...
                // This should ideally not happen if estimatedTocPagesNeeded is accurate
                // but as a fallback, add a new page at the end of the document
                // this will be moved to correct position when pdfDoc.save() is called
      ...
      if (entry.showPage) {
        ...
        const availableSpace = tocPage.getWidth() - leftMargin - titleWidth - pageNumberWidth - 100; // 100 for margins
